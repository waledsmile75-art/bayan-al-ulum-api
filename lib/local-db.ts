...
export async function markDocumentResult(id: number, ok: boolean, error?: string, remoteDocumentId?: number) { const item = memoryDocs.find((doc) => doc.id === id); if (item) { item.status = ok ? "synced" : "failed"; item.lastError = error ?? null; item.remoteDocumentId = remoteDocumentId ?? item.remoteDocumentId; item.attempts += 1; } }
