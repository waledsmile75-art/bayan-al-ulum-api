...
export async function summarizeDocument(question?: string) { return (await request<{ result: { text: string } }>("/api/core/summarize", { method: "POST", body: JSON.stringify({ question }) })).result.text; }
export async function explainDocument(question?: string) { return (await request<{ result: { text: string } }>("/api/core/explain", { method: "POST", body: JSON.stringify({ question }) })).result.text; }
