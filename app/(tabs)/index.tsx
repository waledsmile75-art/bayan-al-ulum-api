...
import { analyzeImage, askCore, getCoreSnapshot, ingestPdf, getConfiguredApiBaseUrl, setConfiguredApiBaseUrl, summarizeDocument, explainDocument, translateDocument, type CoreAnswer, type CoreSnapshot } from "@/lib/core-api";
...
export default function HomeScreen() {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      try {
        ...
        await markDocumentResult(localItem.id, true, undefined, result.documentId);
  ...
  const handleWriting = async (mode: "summary" | "explain") => { if (!selectedDocument?.remoteDocumentId) { Alert.alert("اختر مستندًا مفهرسًا", "ارفع الملف وانتظر اكتمال الفهرسة ثم اختره من قائمة المستندات."); return; } setWriting(mode); setWritingMode(mode); setWritingResult(null); try { setWritingResult(mode === "summary" ? await summarizeDocument(question, selectedDocument.remoteDocumentId) : await explainDocument(question, selectedDocument.remoteDocumentId)); } catch (error) { Alert.alert("تعذر تنفيذ العملية", String(error)); } finally { setWriting(null); } };
  const handleTranslate = async () => { if (!selectedDocument?.remoteDocumentId) { Alert.alert("اختر مستندًا مفهرسًا", "ارفع الملف وانتظر اكتمال الفهرسة ثم اختره من قائمة المستندات."); return; } setWriting("explain"); setWritingMode("explain"); try { setWritingResult(await translateDocument(selectedDocument.remoteDocumentId, targetLanguage)); } catch (error) { Alert.alert("تعذرت الترجمة", String(error)); } finally { setWriting(null); } };
  ...
  const handleAsk = async () => {
    ...
    try {
      ...
      const result = await askCore(question, selectedDocument?.remoteDocumentId);
    ...
    <ScreenContainer className="px-5 pt-5" containerClassName="bg-background">
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <View style={[styles.card, { backgroundColor: colors.surface, borderColor: colors.border }]}>
          ...
          {localDocuments.length > 0 && <View style={[styles.localList, { borderColor: colors.border, backgroundColor: colors.background }]}><Text style={[styles.evidenceHeading, { color: colors.foreground }]}>اختر المصدر</Text>{localDocuments.slice(0, 8).map((doc) => <Pressable key={doc.id} onPress={() => setSelectedDocument(doc)} style={[styles.localItemRow, selectedDocument?.id === doc.id && { backgroundColor: colors.primary }]}><Text style={[styles.localItem, { color: selectedDocument?.id === doc.id ? "#FFFFFF" : colors.muted }]}>{doc.status === "synced" ? "✓" : "◌"} {doc.filename} · {doc.remoteDocumentId ? "مفهرس" : "بانتظار الفهرسة"}</Text></Pressable>)}</View>}
        </View>
        ...
        <View style={[styles.card, { backgroundColor: colors.surface, borderColor: colors.border }]}>
          ...
          <View style={styles.actionRow}><TextInput value={targetLanguage} onChangeText={setTargetLanguage} placeholder="لغة الترجمة" placeholderTextColor={colors.muted} style={[styles.languageInput, { color: colors.foreground, borderColor: colors.border, backgroundColor: colors.background }]} /><Pressable onPress={handleTranslate} disabled={!!writing} style={[styles.smallButton, { backgroundColor: colors.surface, borderColor: colors.border, borderWidth: 1 }]}><Text style={[styles.smallButtonText, { color: colors.foreground }]}>{writing && writingMode === "explain" ? "جارٍ التنفيذ…" : "ترجم المستند"}</Text></Pressable></View>
